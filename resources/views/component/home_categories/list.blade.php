        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex align-items-center justify-content-between">
                    ...
                    <div class="d-flex">
                        <button data-bs-toggle="modal" data-bs-target="#OrderingModel" type="button" class="btn btn-sm btn-outline-secondary rounded-pill ps-3 pe-3 me-2"><i class="bx bx-list-ul"></i>  Ordering</button>
                        <button data-bs-toggle="modal" data-bs-target="#createmodal" type="button" class="btn btn-sm btn-primary rounded-pill ps-3 pe-3"><i class="bx bxs-plus-circle"></i> Add Home Category</button>
                    </div>
                </div>
...
<script>
    ...
    $('#table-data').on('click', '.edit-btn', function (){
        let data_id = $(this).attr('data-id');
        ModalLoadingOn();
        admin.get("{{route('home.category.get.single')}}?data-id="+data_id)
        .then(function (response){
            ModalLoadingOff();
            if(response.status === 200){
               let data = response.data;
